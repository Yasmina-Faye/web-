...
<head>
	...
	<link rel="stylesheet" href="css/goods/head_footer.css" >
								...
								<c:choose>
									<c:when test="${!empty goodsList}">
										<c:forEach items="${goodsList}" var="i">
											<li class="col-md-3">
												<div class="list">
													<a href="goodsdetail?goodsId=${i.goodsId}">
														<img class="img-responsive" src="${i.upLoadImg.imgSrc}" />
													</a>
													<div class="proinfo">
														<h2>
															<a class="text-center" href="goodsdetail?goodsId=${i.goodsId}">${i.goodsName}</a>
														</h2>
														<p>
															<span style='color: red;font-size: 20px'>${i.price}</span>
															<a class="btn btn-danger btn-xs" onclick="addToCart(${i.goodsId})" href="javascript:void(0)" data-toggle="modal" data-target=".bs-example-modal-sm">加入购物车</a>
														</p>
													</div>
												</div>
											</li>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}GoodsList?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}GoodsList?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){

		window.location="${basePath}GoodsList?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		window.location="${basePath}GoodsList?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}GoodsList?action=list&page="+page;
		}
	})

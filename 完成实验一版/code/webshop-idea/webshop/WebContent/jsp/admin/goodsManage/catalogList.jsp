...
<body>
	...
	<div class="container content">
		<div class="funbtn">
			...
			<a class="btn btn-info" href="jsp/admin/goodsManage/catalogAdd.jsp" >增加分类</a>
		</div>
				...
				<c:when test="${!empty catalogList}">
					<c:forEach items="${catalogList}" var="i" varStatus="n">
						<tr>
							...
							<td>${i.catalogName}</td>
							<td class="noClick">
								<a class="btn btn-danger btn-sm" href="CatalogServlet?action=del&id=${i.catalogId}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}CatalogServlet?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}CatalogServlet?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}CatalogServlet?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		
		window.location="${basePath}CatalogServlet?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}CatalogServlet?action=list&page="+page;
		}
	})
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条用户吗？")){
			...
			window.location = "${basePath}CatalogServlet?action=batDel&ids="+ids;

...
<body>
	<c:if test="${!empty goodsMessage}">
		<h3 class="text-center">${goodsMessage}</h3>
	</c:if>
	...
	<div class="container-fulid content">
		<div class="funbtn">
			...
			<a class="btn btn-info" href="GoodsManageServlet?action=addReq">增加商品</a>
			<div class="search col-md-4">
						<div class="input-group">
							<form action="GoodsManageServlet" method="get">
								<input type="hidden" value="seach" name="action">
		     	 				<input style="float: left;width: 160px;" type="text" class="form-control" name="goodsname" placeholder="输入要搜索的名称...">
		       					&nbsp;&nbsp;&nbsp;
		...
		<table class="table table-striped table-hover">
			...
			<c:choose>
				<c:when test="${!empty goodsList}">
					<c:forEach items="${goodsList}" var="i" varStatus="n">
						<tr>
							<td class="noClick"><input type="checkbox" name="choice" value="${i.goodsId}">
							<td>${i.goodsName}</td>
							...
							<td>${i.price}</td>
							<td class="noClick">
								<a class="btn btn-default btn-xs" href="GoodsManageServlet?action=detail&id=${i.goodsId}">详情</a>
								<a class="btn btn-info btn-xs" href="GoodsManageServlet?action=edit&id=${i.goodsId}">修改</a>
								<a class="btn btn-danger btn-xs" href="GoodsManageServlet?action=del&id=${i.goodsId}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}GoodsManageServlet?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}GoodsManageServlet?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}GoodsManageServlet?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		
		window.location="${basePath}GoodsManageServlet?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}GoodsManageServlet?action=list&page="+page;
		}
	})
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条用户吗？")){
			...
			window.location = "${basePath}GoodsManageServlet?action=batDel&ids="+ids;

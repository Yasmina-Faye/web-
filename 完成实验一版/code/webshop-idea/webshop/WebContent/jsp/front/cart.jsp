								...
								<c:forEach items="${shopCart.map}" var="i">
									<tr class="row" id="pro-tr-${i.key}">
										<td><img class="img-responsive col-md-4"
											src="${i.value.goods.upLoadImg.imgSrc }" alt="" />
											<div class="col-md-8">
												<a href="goodsdetail?goodsId=${i.key}">${i.value.goods.goodsName}</a>
												<p>${i.value.goods.origin}</p>
												<p>${i.value.goods.supplier}</p>
											</div>
										</td>
										<td >￥<i>${i.value.goods.price}</i></td>
										<td>
											<div class="spinner">
	  ...
	  //输入框处理
		...
		$("#tosettle").click(function(){
			$.get("UserServlet?action=landstatus",function(data){
				if(data.status=="y"){
					window.location.href="${basePath}jsp/front/conorder.jsp";
				}else{
					$("#myModal").modal("show");
				}
			},"json")
